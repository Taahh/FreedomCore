...
public class MOTDSetter {
	...
	public static void setPlayerCountMsg() {
		...
		// If you want to add more message, copy 'names.add(new
		// WrappedGameProfile("number", "ur message"));'
		// Make sure that 'number' goes in order. Instance:
		// names.add(new WrappedGameProfile("1", ChatColor.LIGHT_PURPLE + "This is
		// message 1!"));
		// names.add(new WrappedGameProfile("2", ChatColor.GREEN + "This is message
		// 2!"));
		// names.add(new WrappedGameProfile("3", ChatColor.GREEN + "This is message
		// 3!"));
		// names.add(new WrappedGameProfile("4", ChatColor.GREEN + "This is message
		// 4!"));
		ProtocolLibrary.getProtocolManager()
				.addPacketListener(new PacketAdapter(FreedomCore.plugin, ListenerPriority.NORMAL,
					...
					public void onPacketSending(PacketEvent event) {
						...
						event.getPacket().getServerPings().read(0)
								.setVersionName(ChatColor.translateAlternateColorCodes('&', versionname));
						event.getPacket().getServerPings().read(0)
								.setMotD(ChatColor.translateAlternateColorCodes('&', motd));
					}
				});
	}
}
